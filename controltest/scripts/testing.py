...
class Ctrtest(Darknetinit, Mavrosinit):
    ...
    def potentialflow(self, ori_x=0, ori_y=0, ori_z=0, x=0, y=0, z=0, mode=0):
        """timeout(int): seconds"""
            # WPT flight
    ...
    def rotationveccal(self, x, y, posx, posy, strength):
        vecx = -strength * (y-posy)/((x-posx)**2 + (y-posy)**2)
        vecy = strength * (x-posx)/((x-posx)**2 + (y-posy)**2)
        return np.array([vecx, vecy])
